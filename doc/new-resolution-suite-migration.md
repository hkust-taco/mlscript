...
2. **Investigate the retained timeout.** Minimize `ups/examples/HindleyMilner.mls`,
   which still times out and terminates SBT on the current JVM. The
   `codegen/Generators.mls` trial now finishes but rejects unresolved `next`
   selections; its earlier timeout was not reproduced.
...
- `ups/examples/HindleyMilner.mls`: Timeout under new resolution; the test runner terminated SBT.
