...
| F1-04 | Done | Investigate support for reserved keywords as field names, especially `val`; implement only if it is a small localized change. If it requires broad parser architecture changes, mark `Skipped` and document why. | `Fix F1` |
| F1-05 | Done | Restore affected Caml Light examples toward their original source where the new support allows it. | `Fix F1` |
...
| W1-02 | Done | Refresh extension diagrams and the precedence table clearly after every successful parse that changes or depends on grammar extensions. | `Web W1` |
...
Phase W1 is complete. Continue with minor web-demo diagnostics task W2-01
