...
| ID | Status | Task | Commit Label |
| --- | --- | --- | --- |
| F1-01 | Pending | Support `mutable` annotations in type-level record fields. | `Fix F1` |
| F1-02 | Pending | Fix the lexer/parser misunderstanding of identifiers like `in_channel`; `in` must not be treated as a keyword prefix inside a longer identifier. | `Fix F1` |
...
Phase P2 is complete and ready to commit. Continue with Phase F1 next unless a
