 ...
 */
private[io] class JsPath(val pathString: String) extends Path:
  ...
  def up: Path = new JsPath(NodePath.dirname(pathString))

  def /(relPath: RelPath): Path =
    new JsPath(NodePath.join(pathString, relPath.toString))
  
  def /(fragment: String): Path =
    new JsPath(pathString + NodePath.sep + fragment)

  def relativeTo(base: Path): Opt[RelPath] =
    try S(new JsRelPath(NodePath.relative(base.toString, pathString)))
 ...
 */
private[io] class JsRelPath(val pathString: String) extends RelPath:
  override def toString: String = pathString
  ...
  def /(other: RelPath): RelPath =
    new JsRelPath(NodePath.join(pathString, other.toString))

/**
 * Platform-specific factory for creating Path instances
 */
private[io] object PathFactory:
  def fromString(str: String) = new JsPath(str)
  def separator: String = NodePath.sep
  def relPathFromString(str: String) = new JsRelPath(str)
  def relPathUp = new JsRelPath("..")
