...
class InvalTyper(using elState: Elaborator.State, tl: TL)(using Ctx):
  ...
  trace[GeneralType](s"${ctx.lvl}. Typing type ${ty.showDbg}", r => s"~> ${r.showDbg}"):
    ...
    case f @ Term.Forall(tvs, outer, body) =>
      val outVar = freshOuter(outer.getOrElse(TempSymbol(S(f), erasedType = N, "outer")))(using ctx)
  ...
  private def instantiate(ty: PolyType)(using ctx: InvalCtx): GeneralType =
    ty.instantiate(infVarState.nextUid, freshEnv(TempSymbol(N, erasedType = N, "env")), ctx.lvl)(tl)
  ...
  private def typeCode(code: Term)(using ctx: InvalCtx, scope: Scope): (Type, Type, Type) =
    ...
    case f @ Lam(PlainParamList(params), body) =>
      ...
      val res = freshVar(TempSymbol(S(f), erasedType = N, "ctx"))(using ctx)
    ...
    case app @ Term.App(lhs, Term.Tup(rhs)) =>
      ...
      val resTy = freshVar(TempSymbol(S(app), erasedType = N, "app"))
    ...
    case unq @ Term.Unquoted(body) =>
      ...
      val tv = freshVar(TempSymbol(S(unq), erasedType = N, "cde"))
      val cr = freshVar(TempSymbol(S(unq), erasedType = N, "ctx"))
    ...
    if sym2 is sym => // TODO: more than one!!
      ...
      val res = freshVar(TempSymbol(S(blk), erasedType = N, "ctx"))(using ctx)
  ...
  private def typeFunDef(sym: Symbol, lam: Term, sig: Opt[Term])(using ctx: InvalCtx, cctx: CCtx, scope: Scope) = lam match
    case Term.Lam(params, body) => sig match
      case S(sig) =>
        ...
        ()
      case N =>
        val outer = freshOuter(TempSymbol(S(lam), erasedType = N, "outer"))(using ctx)
      ...
      case pat: FlatPattern.ClassLike =>
        pat.constructor.symbol.flatMap(_.asCls) match
          case S(sym) =>
            ...
            case S((sym, cls)) =>
              (ClassLikeType(sym, cls.tparams.map(_ => freshWildcard(sym))), (freshVar(TempSymbol(S(scrutinee), erasedType = N, "scrut"))), ClassLikeType(sym, cls.tparams.map(_ => Wildcard.empty)))
            case _ =>
              error(msg"Cannot match ${scrutinee.toString} as ${sym.toString}" -> split.toLoc :: Nil)
              (Bot, Bot, Bot)
  ...
  trace[(GeneralType, Type)](s"${ctx.lvl}. Typing ${t.showDbg}", res => s": (${res._1.showDbg}, ${res._2.showDbg})"):
    ...
    t match
      ...
      case Term.SetRef(lhs, rhs) =>
        ...
        val sk = freshVar(TempSymbol(S(lhs), erasedType = N, "reg"))
      ...
      case Term.Deref(ref) =>
        ...
        val sk = freshVar(TempSymbol(S(ref), erasedType = N, "reg"))
        val ctnt = freshVar(TempSymbol(S(ref), erasedType = N, "ref"))
