...
class Lifter(handlerPaths: Opt[HandlerPaths])(using State, Raise):
  ...
  // firstClsFns was used to eliminate certain useless functions generated by the lifter
  // i.e. the curried function is not needed if a nested function is never used as an object
  // but this should be implemented in a more general dead code elimination pass
  ...
  private def createMetadata(d: Defn, ctx: LifterCtx): LifterMetadata =
    ...
    LifterMetadata(ignored ++ newUnliftable, modules.toList, objects.toList)
    ...
    val walker1 = new BlockTransformerShallow(SymbolSubst()):
      override def applyBlock(b: Block): Block = b match
        case Define(d, rest) =>
          ...
          val LifterMetadata(unliftable, modules, objects) = createMetadata(d, ctx)
