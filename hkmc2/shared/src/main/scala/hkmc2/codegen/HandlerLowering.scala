...
class HandlerPaths(using Elaborator.State):
  val runtimePath: Path = Value.SimpleRef(State.runtimeSymbol)
...
class HandlerLowering(paths: HandlerPaths, opt: EffectHandlers)(using TL, Raise, Elaborator.State, Elaborator.Ctx):
  ...
  private def rtThrowMsg(msg: Str) = Throw(
    Instantiate(mut = false, Value.This(State.globalThisSymbol).selN(Tree.Ident("Error")),
  ...
  private def translateBlock(blk: Block, h: HandlerCtx, scopedVars: collection.Set[Local]): Block =
    ...
    def translateFunLike(fun: FunDefn, funcPath: Path, thisPath: Option[Path], debugNme: Str) =
      ...
      val rtArgLists = intLit(fun.params.length) :: fun.params.flatMap: pl =>
        intLit(pl.params.length) :: pl.params.map(p => Value.SimpleRef(p.sym))
      val newCtx = HandlerCtx.FunctionLike(FunctionCtx(funcPath, thisPath, ResumeInfo(rtArgLists, varList, L(fun.sym)),
        DebugInfo(debugNme, if opt.debug then Value.SimpleRef(debugInfoSym) else unit), thisPath.isDefined && fun.params.isEmpty))
        ...
        case defn @ ClsLikeDefn(owner, isym, sym, ctorSym, kind, paramsOpt, auxParams, parentPath, methods, privateFields, publicFields, preCtor, ctor, companion, bufferable) =>
          ...
          val companion2 = companion.map: bod =>
            ...
            val newCtor = if opt.doNotInstrumentTopLevelModCtor && !h.innerDefIsTrulyNested then bod.ctor else
              translateCtorLike(bod.ctor, Value.This(bod.isym), true)
          ...
          val c2 = ClsLikeDefn(owner, isym, sym, ctorSym, kind, paramsOpt, auxParams, parentPath, newMtds, privateFields, publicFields,
            translateCtorLike(preCtor, Value.This(isym), false), translateCtorLike(ctor, Value.This(isym), false), companion2, bufferable)(defn.configOverride, defn.annotations)
          if opt.debug then
    ...
    def getSaved(off: BigInt): (Block => Block, Path) =
      ...
      val addOne = Assign(getSavedTmp, Call(Value.SimpleRef(State.builtinOpsMap("+")), (paths.runtimePath.selSN("resumeIdx").asArg :: intLit(off).asArg :: Nil) ne_:: Nil)(false, false, false), _)
      (addOne, DynSelect(paths.runtimePath.selSN("resumeArr"), Value.SimpleRef(getSavedTmp), true))

    val resumeArrIndexed = DynSelect(paths.runtimePath.selSN("resumeArr"), Value.SimpleRef(getSavedTmp), true)
    val plus = Value.SimpleRef(State.builtinOpsMap("+"))
    ...
    val restoreVars = vars.zipWithIndex.foldLeft(preRestore):
      case (builder, (local, idx)) => builder
        .assign(getSavedTmp, if idx == 0 then paths.resumeIdx else Call(plus, (Value.SimpleRef(getSavedTmp).asArg :: intLit(1).asArg :: Nil) ne_:: Nil)(false, false, false))
