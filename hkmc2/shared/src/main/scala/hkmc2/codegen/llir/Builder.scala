...
final class LlirBuilder(using Elaborator.State)(tl: TraceLogger, uid: FreshInt):
  ...
  private def bNestedFunDef(e: FunDefn)(k: TrivialExpr => Ctx ?=> Node)(using ctx: Ctx)(using Raise, Scope): Node =
    ...
    if params.length === 0 then
      ...
      val wrappedLambda = params.tail.foldRight(body)((params, acc) => Return(LamRes(params, acc), false))
      bLam(LamRes(fstParams, wrappedLambda), S(sym.nme), S(sym))(k)(using ctx)
  ...
  private def bLam(lam: LamRes, nameHint: Opt[Str], recName: Opt[Local])(k: TrivialExpr => Ctx ?=> Node)(using ctx: Ctx)(using Raise, Scope) : Node =
    trace[Node](s"bLam begin", x => s"bLam end: ${x.show}"):
      val LamRes(params, body) = lam
      ...
      case Value.Ref(l) => 
        ctx.fn_ctx.get(l) match
          case Some(f) =>
            ...
            bLam(LamRes(paramsList, Return(app, false)), S(l.nme), N)(k)
          case None =>
            k(ctx.findName(l) |> sr)
  ...
  private def bResult(r: Result)(k: TrivialExpr => Ctx ?=> Node)(using ctx: Ctx)(using Raise, Scope) : Node =
    trace[Node](s"bResult begin", x => s"bResult end: ${x.show}"):
      ...
      case lam @ LamRes(params, body) => bLam(lam, N, N)(k)
      case ArrRes(false, elems) =>
        bArgs(elems):
          case args: Ls[TrivialExpr] =>
            val v: Local = newTemp
            Node.LetExpr(v, Expr.CtorApp(builtinTuple(elems.length), args), k(v |> sr))
      case RcdRes(mut, fields) => bErrStop(msg"Unsupported value: Rcd")
