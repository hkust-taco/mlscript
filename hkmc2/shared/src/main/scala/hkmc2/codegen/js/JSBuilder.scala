...
class JSBuilder(using TL, State, Ctx) extends CodeBuilder:
  ...
  def result(r: Result)(using Raise, Scope): Document = r match
    ...
    case LamRes(ps, bod) => scope.nest givenIn:
    ...
    case ArrRes(mut, es) if es.isEmpty => if mut then "[]" else doc"$freeze([])"
    case ArrRes(mut, es) =>
      val inner =
