...
class Lowering()(using Config, TL, Raise, State, Ctx):
  ...
  def setupRef(symbol: Local, fresh: Bool)(k: Result => Block)(using Subst): Block =
    setupSymbol(symbol, fresh): r =>
      val l = new TempSymbol(N)
      Assign(l, r, k(Instantiate(Value.Ref(State.termSymbol).selSN("Ref"), Value.Ref(l) :: Nil)))
