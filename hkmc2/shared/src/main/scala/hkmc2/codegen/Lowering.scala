...
class Subst(initMap: Map[(Local, Bool), Value]):
  val map = initMap

  def +(kv: ((Local, Bool), Value)): Subst =
    kv match
    case ((ns: NamedSymbol, _), Value.Ref(ts: TempSymbol)) =>
      ts.nameHints += ns.name
    case _ =>
    Subst(map + kv)

  def get(sym: Local, nextStage: Bool): Opt[Value] = map.get(sym -> nextStage)

  def apply(v: Value): Value = v match
    case Value.Ref(l) => map.getOrElse(l -> false, v)
    case _ => v
object Subst:
...
class Lowering()(using Config, TL, Raise, State, Ctx):
  ...
  def setupTerm(name: Str, args: Ls[Path])(k: Result => Block)(using Subst): Block =
    k(Instantiate(Value.Ref(State.runtimeSymbol).selSN("term").selSN(name), args))

  def setupFreshName(name: Str)(k: Result => Block)(using Subst): Block =
    k(Call(
      Value.Ref(State.runtimeSymbol).selSN("term").selSN("freshName"),
      Value.Lit(Tree.StrLit(name)).asArg :: Nil)(true, true))

  def setupObj(name: Str): Path =
    Value.Ref(State.runtimeSymbol).selSN("term").selSN(name)

  def setupSymbol(symbol: Local, fresh: Bool)(k: Result => Block)(using Subst): Block =
    if fresh then
      setupFreshName(symbol.nme): r =>
        val l = new TempSymbol(N)
        Assign(l, r, k(Instantiate(Value.Ref(State.runtimeSymbol).selSN("term").selSN("Symbol"), Value.Ref(l) :: Nil)))
    else
      k(Instantiate(Value.Ref(State.runtimeSymbol).selSN("term").selSN("Symbol"), Value.Lit(Tree.StrLit(symbol.nme)) :: Nil))
  ...
  def quote(t: st)(k: Result => Block)(using Subst): Block = t match
    ...
    case Ref(sym) if Subst.builtinOps.contains(sym.nme) => // builtin symbols
      val l = new TempSymbol(N)
      setupTerm("Builtin", Value.Lit(Tree.StrLit(sym.nme)) :: Nil)(k)
    case Ref(sym) => // intermediate temp variables generated by ucs
      val subst = summon[Subst]
      subst.get(sym, true) match
        case S(v) =>
          setupTerm("Ref", v :: Nil)(k)
        case N =>
          raise(ErrorReport(
            msg"Cannot refer to next-stage symbol ${sym.nme}." ->
            t.toLoc :: Nil,
            source = Diagnostic.Source.Compilation
          ))
          End("error")
    ...
    case Lam(params, body) =>
      def rec(ps: Ls[LocalSymbol & NamedSymbol], ds: Ls[Path])(k: Result => Block)(using Subst): Block = ps match
        ...
        case sym :: rest =>
          setupSymbol(sym, true): r =>
            val subst = summon[Subst]
            val l = new TempSymbol(N)
            val nest = subst + ((sym -> false) -> Value.Ref(l))
