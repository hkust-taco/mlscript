...
case class Wildcard(in: Type, out: Type) extends TypeArg {
  ...
  override def show(using Scope, Raise): Str = in match
    case `out` => in.show
    case Bot =>
      out match
        case Top => "?"
...
sealed abstract class Type extends GeneralType with TypeArg:
  ...
  protected[bbml] def paren(using Scope, Raise): Str = toBasic match
...
sealed abstract class BasicType extends Type:
  ...
  override def show(using scope: Scope, raise: Raise): Str =
...
case class PolyFunType(args: Ls[GeneralType], ret: GeneralType, eff: Type) extends GeneralType:
  ...
  override def show(using Scope, Raise): Str = s"(${args.map(_.show).mkString(", ")}) ->{${eff.show}} ${ret.show}"
