...
object Parser:
  ...
  object Indent_Curly:
    def unapply(t: Curly.type | Indent.type): true = true
  
  object NEWLINE_COMMA:
    def unapply(t: NEWLINE.type | COMMA.type): true = true
...
abstract class Parser(
  ...
  private def expandSuspensions(tokens: Ls[TokLoc]): Ls[TokLoc] = tokens match
    case (SUSPENSION(true), l0) :: (NEWLINE_COMMA(), l1) :: rest =>
  ...
  def blockOfImpl(rule: ParseRule[Tree], annotations: Ls[Tree], allowNewlines: Bool): Ls[Tree] =
    ...
    case (tok @ (id: IDENT), loc) :: _ if id.name =/= ":" =>
      ...
      case S(kw) =>
        ...
        case S(subRule) =>
          yeetSpaces match
          case (tok @ BRACKETS(Indent_Curly(), toks), loc) :: _ if subRule.blkAlt.isEmpty =>
            consume
            ...
            case (tok @ BRACKETS(Indent_Curly(), toks), loc) :: _ /* if subRule.blkAlt.isEmpty */ =>
              ...
              case S(subRule) if subRule.blkAlt.isEmpty =>
                rec(toks, S(tok.innerLoc), tok.describe).concludeWith { p =>
                  p.blockOf(subRule.map(e => parseRule(CommaPrecNext, exprAlt.rest, allowNewlines = false).map(res => exprAlt.k(e, res)).getOrElse(errExpr)), annotations, allowNewlines)
                } ++ blockContOf(rule)
              case _ =>
                TODO(cur)
            case _ =>
  ...
  def parseRuleImpl[A](prec: Int, rule: ParseRule[A], allowNewlines: Bool): Opt[A] =
    ...
    case (tok @ (id: IDENT), loc) :: _ =>
      ...
      case S(kw) =>
        ...
        case S(subRule) =>
          ...
          case (tok @ BRACKETS(Indent_Curly(), toks), loc) :: _ if subRule.blkAlt.isEmpty =>
            consume
            rec(toks, S(tok.innerLoc), tok.describe).concludeWith(_.parseRule(kw.rightPrecOrMax, subRule, allowNewlines = false))
          case _ =>
        ...
        case N =>
          ...
          case S(exprAlt) =>
            ...
            case S(subRule) =>
              // parse(subRule)
              val e = exprCont(parseRule(kw.rightPrecOrMin, subRule, allowNewlines = allowNewlines).getOrElse(errExpr), prec, false)
    ...
    case (tok @ (NEWLINE_COMMA()), l0) :: (id: IDENT, l1) :: _ if allowNewlines && rule.kwAlts.contains(id.name) =>
      consume
      parseRule(prec, rule, allowNewlines = allowNewlines)
    case (tok @ (NEWLINE_COMMA()), l0) :: _ =>
      // TODO(cur)
      rule.emptyAlt match
        case S(res) => S(res())
        case N =>
          ...
          N
    case (br @ BRACKETS(Indent_Curly(), toks), loc) :: _ =>
      // rule.blkAlt match
      //   case S(res) => S(res)
      //   case N =>
      //    err((msg"Expected ${rule.whatComesAfter} ${rule.mkAfterStr}; found ${tok.describe} instead" -> lastLoc :: Nil)
      ...
      rule.blkAlt match
        case S(exprAlt) =>
          ...
          // parseRule(prec, exprAlt.rest, allowNewlines = false).map(res => exprAlt.k(e, res))
        case N =>
  ...
  // def expr(prec: Int, allowNewlines: Bool)(using Line): Tree =
    val allowNewlines = allowIndentedBlock // FIXME??
    val res = parseRule(prec,
        if allowNewlines then prefixRulesAllowIndentedBlock else prefixRules,
  ...
  def simpleExprImpl(prec: Int, allowNewlines: Bool): Tree =
    ...
    case (BRACKETS(Indent_Curly(), _), loc) :: _ =>
  ...
  @tailrec final private def splitItem(acc: Ls[Tree]): Ls[Tree] =
    val item = wrap(s"index = ${acc.size + 1}"):
      ...
      case Nil => false
      case (NEWLINE_COMMA() | SPACE, _) :: _ => consume; true
  ...
  */
  ...
  def opSplitImpl(lhs: Tree, splittingOpLoc: Loc, prec: Int, acc: Ls[Tree]): Tree =
    ...
    case (IDENT(op, true), l0) :: rest =>
      ...
      if rest.collectFirst{ case (NEWLINE_COMMA() | IDENT("then", false), _) => }.isEmpty // TODO dedup
      then
  ...
  final def exprContImpl(acc: Tree, prec: Int, allowNewlines: Bool): Tree =
    cur match
      case (QUOTE, l) :: _ => cur match {
        ...
        case _ :: (OP(opStr), l0) :: _ =>
          if opPrec(opStr)._1 > prec then {
            ...
            yeetSpaces match {
              case (NEWLINE_COMMA(), l0) :: _ => consume
              case _ =>
            }
      ...
      case (COMMA, l0) :: _ if prec === 0 =>
        ...
        acc
        /* 
      case (KEYWORD(opStr @ "=>"), l0) :: (NEWLINE_COMMA(), l1) :: _ if opPrec(opStr)._1 > prec =>
        consume
      ...
      case (OP(opStr), l0) :: _ if /* isInfix(opStr) && */ opPrec(opStr)._1 > prec =>
        ...
        yeetSpaces match
          case (BRACKETS(Indent_Curly(), toks), l0) :: _ =>
            consume
            // rec(toks, S(br.innerLoc), br.describe).concludeWith(f(_, true))
      ...
      */
      ...
      case (NEWLINE_COMMA(), _) :: (KEYWORD(kw), _) :: _
      ...
      case (br @ BRACKETS(bk @ (Indent_Curly()), toks @ ((KEYWORD(kw), _) :: _)), loc) :: _
      if kw.leftPrecOrMin > prec
      && infixRules.kwAlts.contains(kw.name)
      =>
        consume
          ...
          case S(rule) =>
            ...
            rule.exprAlt match
              case S(exprAlt) =>
                ...
                parseRule(kw.rightPrecOrMin, exprAlt.rest, allowNewlines = false).map: rest =>
                  exprCont(exprAlt.k(rhs, rest)(acc), prec, allowNewlines = false) // FIXME prec??
                .getOrElse(errExpr)
              case N =>
                // TODO other alts...?
  ...
  final def exprJuxImpl(acc: Tree, prec: Int, allowNewlines: Bool): Tree =
    cur match
    case (NEWLINE_COMMA(), _) :: _ if allowNewlines =>
      consume
    ...
    case (br @ BRACKETS(Indent_Curly(), toks), l0) :: _
