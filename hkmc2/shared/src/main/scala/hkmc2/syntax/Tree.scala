...
enum Tree extends AutoLocated:
  ...
  def children: Vector[Located] = this match
    ...
    case Pun(_, e) => Vector(e)
    case Bra(_, e) => Vector(e)
    ...
    case Modified(_, body) => Vector(body)
    case Quoted(body) => Vector(body)
    case Unquoted(body) => Vector(body)
    case Tup(fields) => fields.toVector
    case App(lhs, rhs) => Vector(lhs, rhs)
    case OpApp(lhs, op, rhss) => lhs +: op +: rhss.toVector
    case Jux(lhs, rhs) => Vector(lhs, rhs)
    case PrefixApp(kw, rhs) => Vector(kw, rhs)
    case InfixApp(lhs, kw, rhs) => Vector(lhs, kw, rhs)
    ...
    case DynamicNew(body) => Vector(body)
    case IfLike(_, split) => Vector(split)
    case Case(_, bs) => Vector(bs)
    case Region(name, body) => Vector(name, body)
    case RegRef(reg, value) => Vector(reg, value)
    case Effectful(eff, body) => Vector(eff, body)
    case Outer(name) => name.toVector
    case TyTup(tys) => tys.toVector
    case Sel(prefix, name) => Vector(prefix)
    case SynthSel(prefix, name) => Vector(prefix)
    case DynAccess(prefix, fld) => Vector(prefix, fld)
    case Open(bod) => Vector(bod)
    case OpenIn(opened, body) => Vector(opened, body)
    case Def(lhs, rhs) => Vector(lhs, rhs)
    case Spread(kw, body) => Vector(kw) ++ body.toVector
    case Annotated(annotation, target) => Vector(annotation, target)
    case Constructor(decl) => Vector(decl)
    case MemberProj(cls, name) => Vector(cls)
