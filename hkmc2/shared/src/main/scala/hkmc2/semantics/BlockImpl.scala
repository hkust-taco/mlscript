  ...
  val desugStmts =
    def desug(stmts: Ls[Tree]): Ls[Tree] =
      ...
      case stmt :: stmts =>
        ...
        case TypeDef(syntax.Cls, Ident(name), rhs, S(Block(Constructor(Block(ctors)) :: rest))) =>
          TypeDef(syntax.Cls, Ident(name), rhs, if rest.isEmpty then N else S(Block(rest))) ::
            (ctors.map(head => TypeDef(syntax.Cls, InfixApp(head, syntax.Keyword.`extends`, Ident(name)), N, N)))
          ::: desug(stmts)
