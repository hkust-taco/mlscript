...
trait DesugaringBase(using state: Elaborator.State):
  ...
  import elaborator.{term, cls}, elaborator.tl.*, state.globalThisSymbol
  ...
  protected def resolvePredefMember(name: Str): Ctxl[(Term.SynthSel, ClassSymbol)] =
    ...
    val innerSel = Term.SynthSel(state.globalThisSymbol.ref(), Ident("Predef"))(S(predefSymbol))
    ...
    (Term.SynthSel(innerSel, Ident(name))(S(memberSymbol)), memberSymbol)
  ...
  protected lazy val stringStartsWith: Ctxl[Term] =
    term(SynthSel(SynthSel(Ident("globalThis"), Ident("Predef")), Ident("stringStartsWith")))

  protected lazy val stringGet: Ctxl[Term] =
    term(SynthSel(SynthSel(Ident("globalThis"), Ident("Predef")), Ident("stringGet")))

  protected lazy val stringDrop: Ctxl[Term] =
    term(SynthSel(SynthSel(Ident("globalThis"), Ident("Predef")), Ident("stringDrop")))
