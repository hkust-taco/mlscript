...
class Normalization(lowering: Lowering)(using tl: TL)(using Raise, Ctx, State) extends TermSynthesizer:
  ...
  private def apply(inputSplit: Split, kw: `if`.type | `while`.type, t: Opt[Term], k: Result => Block)(using Config, LoweringCtx) =
    ...
    lazy val fDSym = new TermSymbol(syntax.Fun, N, Tree.Ident(f.nme))
    ...
    val loopCont = if config.rewriteWhileLoops
      then Return(Call(Value.Ref(f, S(fDSym)), Nil)(true, true, false), false)
      ...
      if kw === `while` then
        if config.rewriteWhileLoops then
          ...
          val blk = blockBuilder
            ...
            .define(FunDefn(N, f, fDSym, PlainParamList(Nil) :: Nil, Begin(body, Return(loopEnd, false)))(isTailRec = false))
