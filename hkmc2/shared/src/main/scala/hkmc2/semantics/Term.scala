...
enum Term extends Statement:
  ...
  /** The set of free variable names in this term.
    * Accounts for local bindings introduced by `let` declarations (in `Blk`),
    * lambda parameters, `TermDefinition` names, class/module names and their
    * constructor parameters, and pattern variables in `IfLike` splits. */
...
object Term:
    ...
    stats.foldRight(resFree): (stat, acc) =>
      stat match
        ...
        case td: TermDefinition =>
          (acc - td.sym.nme) ++ termDefFreeVars(td)
        case cls: ClassLikeDef =>
          (acc - cls.sym.nme) ++ classLikeDefFreeVars(cls)
        case td: TypeDef =>
          (acc - td.bsym.nme) ++ td.rhs.iterator.flatMap(_.freeVars)
        case pat: PatternDef =>
          (acc - pat.bsym.nme) ++ patternDefFreeVars(pat)
        case t: Term =>
          acc ++ t.freeVars
        case _: (Import | SetConfig | RcdField | RcdSpread) =>
  ...
  /** Free vars of a PatternDef body, with constructor params subtracted. */
  private def patternDefFreeVars(pat: PatternDef): Set[Str] =
    val paramNames = pat.paramsOpt.iterator.flatMap(_.allParams).map(_.sym.nme).toSet
    pat.body.blk.freeVars -- paramNames
end Term
