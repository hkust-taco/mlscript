    ...
    val parsed = collectUntilPrompt() match
      case reply: Str =>
        ...
        if begin >= 0 && end >= 0 then
          ...
          ReplHost.Error(false, reply.substring(begin + 1, end), reply)
        else reply
...
object ReplHost {
  ...
  /**
    ...
    * @param message the error message
    */
  final case class Error(syntax: Bool, message: Str, allOutput: Str) extends Reply {
    override def map(f: Str => Reply): Reply = this
