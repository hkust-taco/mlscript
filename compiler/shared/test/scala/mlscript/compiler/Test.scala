...
class DiffTestCompiler extends DiffTests {
  ...
  override def postProcess(mode: ModeType, basePath: List[Str], testName: Str, unit: TypingUnit, output: Str => Unit): (List[Str], Option[TypingUnit]) = 
    ...
    if(mode.polydef) {
      ...
      // output(pd.termToProdType.toString())
      // output(s"TypeVars:\n${pd.varsName.mkString("\n")}")
      // output(s"Upper bounds:\n${pd.upperBounds.mkString("\n")}")
      // output(s"Lower bounds:\n${pd.lowerBounds.mkString("\n")}")
      // output(s"Selection sources:\n${pd.selectionSource.mkString("\n")}")
      // output(s"Obj Destinations:\n${pd.objDestination.mkString("\n")}")
