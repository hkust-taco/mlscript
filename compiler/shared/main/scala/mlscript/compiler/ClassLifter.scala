  ...
  private def liftTerm(target: Term)(using ctx: LocalContext, cache: ClassCache, globFuncs: Map[Var, (Var, LocalContext)], outer: Option[ClassInfoCache]): (Term, LocalContext) = 
    ...
    case Lam(lhs, rhs) => 
      ...
      val anoCls = NuTypeDef(
        ...
        TypingUnit(List(NuFunDef(None, Var("apply"), N, Nil, Left(Lam(lhs, rhs)))(N, N, N, N, N, false))))(N, N, N) //TODO: Use Proper Arguments
  ...
  private def liftMemberFunc(func: NuFunDef)(using ctx: LocalContext, cache: ClassCache, globFuncs: Map[Var, (Var, LocalContext)], outer: Option[ClassInfoCache]): (NuFunDef, LocalContext) = {
    ...
    body match{
      case Left(Lam(lhs@Tup(etts), rhs)) =>
        ...
        (func.copy(rhs = Left(Lam(lret._1, ret._1)))(func.declareLoc, func.exportLoc, func.virtualLoc, func.signature, func.outer, func.genField), ret._2 -+ lret._2) //TODO: Check correctness
      case Left(value) => 
        // will be treated as Lam(Tup(Nil), rhs)
        val ret = liftTerm(value)(using ctx.addT(tpVs))
        (func.copy(rhs = Left(Lam(Tup(Nil), ret._1)))(func.declareLoc, func.exportLoc, func.virtualLoc, func.signature, func.outer, func.genField), ret._2) //TODO: Check correctness
      case Right(PolyType(targs, body)) => 
        ...
        (func.copy(rhs = Right(PolyType(nTargs._1, nBody._1)))(func.declareLoc, func.exportLoc, func.virtualLoc, func.signature, func.outer, func.genField),
