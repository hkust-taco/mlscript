    ...
    term match {
      ...
      case App(f, a) =>
        ...
        val a_ty = {
          ...
          a match {
            case tup @ Tup(as) =>
              TupleType(as.map { case (n, Fld(FldFlags(mut, spec, opt), a)) => // TODO handle mut?
                // assert(!mut)
  ...
  def expandType(st: TypeLike, stopAtTyVars: Bool = false)(implicit ctx: Ctx): mlscript.TypeLike = {
    ...
    def goDecl(d: NuMember)(implicit ectx: ExpCtx): NuDecl = d match {
      ...
      case tf @ TypedNuFun(level, fd, bodyTy) =>
        NuFunDef(fd.isLetRec, fd.nme, fd.symbolicNme, Nil, R(go(tf.typeSignature)))(fd.declareLoc, fd.signature, fd.outer)
      case p: NuParam =>
        ??? // TODO
      case TypedNuDummy(d) =>
        ??? // TODO
