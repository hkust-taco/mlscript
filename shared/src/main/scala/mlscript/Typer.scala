...
/** A class encapsulating type inference state. It uses its own internal representation of types and type variables,
  * using mutable data structures. Inferred SimpleType values are then turned into CompactType values for
  * simplification. In order to turn the resulting CompactType into a mlscript.Type, we use `expandCompactType`.
  */
class Typer(var dbg: Boolean, var verbose: Bool, var explainErrors: Bool) extends ConstraintSolver with TypeSimplifier {
      ...
      baseClasses.map(v => Var(v.name.toList.mapHead(_.toLower).mkString)) ++
        baseClasses.iterator
          ...
          .flatMap(v =>
            ctx.tyDefs
              .get(v.name)
              .fold(Set.empty[Var])(_.allBaseClasses(ctx)(traversed + v))
          )
  ...
  /** targsMaps: stores the map from type parameters to type variables for polymorphic classes and methods first layer:
    * class name; second layer: method name or None for top-level class type parameters; third layer: type parameter.
    * For "normal" contexts, all innermost values should be `TypeVariable`s. For the nested context during method
    * typing, the innermost values can also be `TraitTag`s.
    */
  case class Ctx(
      parent: Opt[Ctx],
      env: MutMap[Str, TypeScheme],
      lvl: Int,
      inPattern: Bool,
      tyDefs: Map[Str, TypeDef],
      mthDecls: Map[Str, Map[Str, PolymorphicType]],
      mthDefs: Map[Str, Map[Str, PolymorphicType]],
      methodBase: MutMap[Str, Option[Var]],
    ...
    def get(name: Str): Opt[TypeScheme] =
    ...
    def allBaseClassesOf(name: Str): Set[Var] = abcCache.getOrElse(
      name,
      tyDefs
        .get(name)
        .fold(Set.empty[Var])(
          _.allBaseClasses(this)(Set.empty).tap(abcCache += name -> _)
        )
    )

    private val thisTyCache = MutMap.empty[Str, TypeRef]

    def thisType(tn: Str)(implicit prov: TypeProvenance): TypeRef =
      thisTyCache.getOrElse(
        tn, {
          val td = tyDefs(tn)
  ...
  val TrueType: ClassTag =
    ClassTag(Var("true"), Set.single(Var("bool")))(noProv)

  val FalseType: ClassTag =
    ClassTag(Var("false"), Set.single(Var("bool")))(noProv)

  val IntType: ClassTag =
    ClassTag(Var("int"), Set.single(Var("number")))(noProv)
  ...
  val builtinBindings: Bindings = {
    ...
    val intBinPred =
      fun(singleTup(IntType), fun(singleTup(IntType), BoolType)(noProv))(noProv)
    Map(
      "true"     -> TrueType,
      "false"    -> FalseType,
      "document" -> BotType,
      ...
      "concat" -> fun(
        singleTup(StrType),
        fun(singleTup(StrType), StrType)(noProv)
      )(noProv),
      "eq" -> {
        val v = freshVar(noProv)(1)
        PolymorphicType(
          0,
          fun(singleTup(v), fun(singleTup(v), BoolType)(noProv))(noProv)
        )
      },
      "ne" -> {
        val v = freshVar(noProv)(1)
      ...
      "&&" -> fun(
        singleTup(BoolType),
        fun(singleTup(BoolType), BoolType)(noProv)
      )(noProv),
      "||" -> fun(
        singleTup(BoolType),
        fun(singleTup(BoolType), BoolType)(noProv)
      ...
      "if" -> {
        ...
        PolymorphicType(
          0,
          fun(BoolType, fun(v, fun(v, v)(noProv))(noProv))(noProv)
        )
      }
  ...
  def clsNameToNomTag(
      td: TypeDef
  )(prov: TypeProvenance, ctx: Ctx): SimpleType = {
    require(td.kind.is(Cls))
    ClassTag(
      Var(td.nme.name.head.toLower.toString + td.nme.name.tail),
      td.allBaseClasses(ctx)(Set.single(Var(td.nme.name)))
    )(prov)
  }
  ...
  /** Extract the mapping of type arguments applied to the base classes and traits from the body of a type definition.
    ...
    */
  def targsAppOf(tyDef: TypeDef)(implicit
      ctx: Ctx,
      raise: Raise,
      vars: Map[Str, SimpleType] = Map.empty
  ): Map[Str, Map[Str, SimpleType]] = {
    def rec(ty: Type): Map[Str, Map[Str, SimpleType]] = ty match {
      case Inter(l, r) => rec(l) ++ rec(r)
      case AppliedType(TypeName(bn), ts) =>
        ctx.tyDefs.get(bn).fold(Map(bn -> Map.empty[Str, SimpleType])) { td =>
          Map(
            bn -> td.tparams
              .map(_.name)
              .zip(ts.map(typeType(_)(ctx.nextLevel, raise, vars)))
              .toMap
          )
        }
  ...
  def wrapMethod(tn: Str, bodyTy: PolymorphicType)(implicit
      prov: TypeProvenance,
      ctx: Ctx
  ): PolymorphicType =
    PolymorphicType(
      bodyTy.level,
      FunctionType(singleTup(ctx.thisType(tn)), bodyTy.body)(prov)
    )

  /** Only supports getting the fields of a valid base class type. Notably, does not traverse type variables.
    */
  def fieldsOf(ty: SimpleType, paramTags: Bool)(implicit
      raise: Raise
  ): Map[Var, ST] =
    // trace(s"Fields of $ty {${travsersed.mkString(",")}}")
    {
      ty match {
        ...
        case Without(base, ns) =>
          fieldsOf(base, paramTags).filter(ns contains _._1)
        case TypeBounds(lb, ub) => fieldsOf(ub, paramTags)
        case _: ObjectTag | _: FunctionType | _: TupleType | _: TypeVariable | _: NegType | _: ExtrType |
            _: ComposedType =>
          Map.empty
      }
    }
  ...
  def processTypeDefs(
      newDefs0: List[mlscript.TypeDef]
  )(implicit ctx: Ctx, raise: Raise): Ctx = {
    ...
    val newDefs = newDefs0.map { td =>
      ...
      td.tparams.groupBy(_.name).foreach {
        case s -> tps if tps.size > 1 =>
          err(
      ...
      val dummyTargs = td.tparams.map(p =>
        freshVar(
          originProv(p.toLoc, s"${td.kind.str} type parameter"),
          S(p.name)
        )(ctx.lvl + 1)
      )
      val td1 = TypeDef(
    ...
    def getSubsMap(tn: Str)(implicit
        ctx: Ctx
    ): Map[Str, Map[SimpleType, SimpleType]] = subsMapCache.getOrElse(
      tn,
      (ctx.tyDefs.get(tn) match {
        case S(td) =>
          val targsApp = targsAppOf(td)(ctx, raise, ctx.tyDefs(tn).targsMap)
          td.baseClasses.flatMap { case Var(bn) =>
            ctx.tyDefs.get(bn).map(_.targsMap).map(bn -> _)
          }.map { case bn -> m =>
            bn -> m.map[SimpleType, SimpleType] {
              case targ -> (tv: TypeVariable) =>
                tv -> targsApp(bn).getOrElse(
                  targ,
                  freshVar(noProv)(tv.level)
                ) // Q: Is this `freshVar` legit? Seems it should never happen
              case _ => die
            }
          }.toMap
    ...
    def typeTypeDefs(implicit ctx: Ctx): Ctx =
      ctx.copy(tyDefs = oldDefs ++ newDefs.flatMap { td =>
        ...
        val n                             = td.nme
        val body_ty =
          typeType(td.body, simplify = false)(ctx.nextLevel, raise, td.targsMap)
        def allDeclMths(td: TypeDef): Set[Str] = td.baseClasses.flatMap(bc =>
          ctx.mthDecls
            .get(bc.name)
            .map(_.keySet)
            .getOrElse(allDeclMths(ctx.tyDefs(bc.name)))
        ) ++ td.mthDecls.map(_.nme.name)
        def allDefMths(td: TypeDef): Set[Str] = td.baseClasses.flatMap(bc =>
          ctx.mthDefs
            .get(bc.name)
            .map(_.keySet)
            .getOrElse(allDefMths(ctx.tyDefs(bc.name)))
        ) ++ td.mthDefs.map(_.nme.name)
        def checkCycle(ty: SimpleType)(implicit
            travsersed: Set[TypeDef \/ TV]
        ): Bool =
          ...
          ty match {
            case TypeRef(td, _) if travsersed(L(td)) =>
              ...
              false
            case tr @ TypeRef(td, targs) =>
              checkCycle(tr.expand)(travsersed + L(td))
            ...
            case tv: TypeVariable =>
              travsersed(R(tv)) || {
                ...
                tv.lowerBounds.forall(checkCycle(_)(t2)) && tv.upperBounds
                  .forall(checkCycle(_)(t2))
              }
            case _: ExtrType | _: ObjectTag | _: FunctionType | _: RecordType | _: TupleType =>
              true
          }
        // }()
        val rightParents = td.kind match {
          ...
          case k: ObjDefKind =>
                  ...
                  case Cls =>
                    if (td.kind.is(Cls)) {
                      (parentsClasses.isEmpty || {
                        err(
                          msg"${td.kind.str} $n cannot inherit from class ${tr.defn.nme} as it already inherits from class ${parentsClasses.head.defn.nme}",
                          prov.loco
                        )
                        false
            ...
            checkParents(body_ty) && checkCycle(body_ty)(
              Set.single(L(td))
            ) && isAbstract.|| {
              val fields = fieldsOf(body_ty, true)
              val tparamTags = td.tparams
                .lazyZip(td.targs)
                .map((tp, tv) => tparamField(td.nme, tp) -> FunctionType(tv, tv)(noProv))
                .toList
              val ctor = k match {
                case Cls =>
                  val nomTag =
                    clsNameToNomTag(td)(originProv(td.nme.toLoc, "class"), ctx)
                  val fieldsRefined = fields.iterator
                    .map(f =>
                      if (f._1.name.isCapitalized) f
                        ...
                        f._1 ->
                          freshVar(
                            noProv,
                            S(
                              f._1.name.drop(f._1.name.indexOf('#') + 1)
                            ) // strip any "...#" prefix
                  ...
                  PolymorphicType(
                    ...
                    FunctionType(
                      singleTup(
                        RecordType.mk(
                          fieldsRefined.filterNot(_._1.name.isCapitalized)
                        )(noProv)
                      ),
                ...
                case Trt =>
                  val nomTag =
                    trtNameToNomTag(td)(originProv(td.nme.toLoc, "trait"), ctx)
                  val tv = freshVar(noProv)(1)
                  ...
                  PolymorphicType(
                    ...
                    FunctionType(
                      singleTup(tv),
                      tv & nomTag & RecordType.mk(tparamTags)(noProv)
        ...
        def checkRegular(ty: SimpleType)(implicit
            reached: Map[Str, Ls[SimpleType]]
        ): Bool = ty match {
          case tr @ TypeRef(defn, targs) =>
            reached.get(defn.nme.name) match {
              case None =>
                checkRegular(tr.expand)(reached + (defn.nme.name -> targs))
              case Some(tys) =>
                // TODO less syntactic check...
                if (defn.nme === td.nme && tys =/= targs) {
                  if (defn.nme.name === td.nme.name)
                    err(
                      msg"Type definition is not regular: it occurs within itself as ${expandType(
                        tr,
                        true
                      ).show}, but is defined as ${expandType(TypeRef(defn, td.targs)(noProv, ctx), true).show}",
                      td.toLoc
                    )(raise, noProv)
                  false
                } else true
    ...
    def typeMethods(implicit ctx: Ctx): Ctx = {
      ...
      newDefs.filter(td => ctx.tyDefs.isDefinedAt(td.nme.name)).foreach { td =>
        ...
        val rigid = td.targsMap.map { case pn -> tv => tv -> targsMap(pn) }
        ...
        (td.mthDecls ++ td.mthDefs).foreach { case md @ MethodDef(rec, prt, nme, tparams, rhs) =>
          implicit val prov: TypeProvenance = tp(
            md.toLoc,
            rhs.fold(_ => "method definition", _ => "method declaration")
          )
          ...
          rhs.fold(_ => defs, _ => decls) += nme.name -> (bodyTy match {
            case PolymorphicType(level, body) =>
              PolymorphicType(level, ProvType(body)(prov))
          })
        ...
        newMthDecls += tn.name ->
          ((td.baseClasses
            .flatMap(t =>
              newMthDecls
                .getOrElse(t.name, Map.empty)
                .view
                .mapValues(subst(_, subsMap(t.name)))
            ))
            .groupMapReduce(_._1)(_._2) { case PolymorphicType(_, body1) -> PolymorphicType(_, body2) =>
              PolymorphicType(
                thisCtx.lvl,
                ComposedType(false, body1, body2)(prov)
              )
            } ++ decls.toMap)
        newMthDefs += tn.name ->
          ((td.baseClasses
        ...
        allEnv ++= (newMthDecls(tn.name) ++ newMthDefs(tn.name).iterator.filter { case mn -> _ =>
          !mthDeclsSet(mn)
          ...
          s"${tn.name}.$mn" -> m_ty ::
            (ctx.methodBase.get(mn) match {
              ...
              case S(S(v))
                  if ctx
                    .allBaseClassesOf(
                      v.name.toList.mapHead(_.toUpper).mkString
                    )
                    .contains(
                      Var(tn.name.toList.mapHead(_.toLower).mkString)
                    ) =>
                ctx.methodBase += mn -> S(
                  Var(tn.name.toList.mapHead(_.toLower).mkString)
                )
      ...
      ctx
        .copy(mthDecls = newMthDecls.toMap, mthDefs = newMthDefs.toMap)
        .tap(checkSubsume(_))
    }
      ...
      newDefs.filter(td => ctx.tyDefs.isDefinedAt(td.nme.name)).foreach { td =>
        ...
        val rigidSubsMap = td.targsMap.map {
          case s -> (tv: TypeVariable) =>
            tv -> freshenAbove(ctx.lvl, tv, true)
          case _ => die
        }
          .toMap[SimpleType, SimpleType]
        val subsMap = getSubsMap(tn.name)
        def ss(
            ...
            bmt: TypeScheme,
            bno: Option[Str] = N
        )(implicit raise: Raise, prov: TypeProvenance) = bmt match {
          case pt: PolymorphicType =>
            constrain(
              subst(mt, rigidSubsMap).instantiate,
              subst(
                bno.fold(pt)(bn => subst(pt, subsMap(bn))),
                rigidSubsMap
              ).rigidify
            )(raise, prov, ctx)
          case st: SimpleType =>
            constrain(
              subst(mt, rigidSubsMap).instantiate,
              subst(bno.fold(st)(bn => subst(st, subsMap(bn))), rigidSubsMap)
            )(raise, prov, ctx)
        }
        decls.foreach { case nme -> mt =>
          ...
          td.baseClasses.foreach { case Var(bn) =>
            ctx.mthDecls.get(bn).foreach(_.get(nme).foreach(ss(mt, _, S(bn))))
          }
        }
        ...
        (td.mthDecls ++ td.mthDefs).foreach { case md @ MethodDef(rec, prt, nme, tparams, rhs) =>
          ...
          td.baseClasses.foreach { case Var(bn) =>
            ctx.mthDefs
              .get(bn)
              .foreach(_.get(nme.name).foreach { bmt =>
                if (!ctx.mthDecls.get(bn).exists(_.isDefinedAt(nme.name))) {
                  err(
                    msg"Overriding method $bn.${nme.name} without explicit declaration is not allowed.",
                    md.toLoc
                  )
                  println(
                    s">> Checking subsumption (against inferred type) for inferred type of $nme : $mt"
                  )
                  ss(mt, bmt)
                }
              })
  ...
  def typeType(ty: Type, simplify: Bool = true)(implicit
      ctx: Ctx,
      raise: Raise,
      vars: Map[Str, SimpleType] = Map.empty
  ): SimpleType = {
    ...
    def typeNamed(loc: Opt[Loc], name: Str): (() => ST) \/ TypeDef =
      ctx.tyDefs
        .get(name)
        .toRight(() =>
          err("type identifier not found: " + name, loc)(
            raise,
            tp(loc, "missing type")
          )
        )
    ...
    def rec(
      ...
      ty match {
        ...
        case Tuple(fields) =>
          TupleType(fields.map(f => f._1 -> rec(f._2)))(fields match {
            case Nil | ((N, _) :: Nil) => noProv
            case _                     => tp(ty.toLoc, "tuple type")
          })
        case Inter(lhs, rhs) =>
          (if (simplify) rec(lhs) & (rec(rhs), _: TypeProvenance)
        ...
        case Record(fs) =>
          ...
          fs.groupMap(_._1.name)(_._1).foreach {
            case s -> fieldNames if fieldNames.size > 1 =>
              err(
                msg"Multiple declarations of field name $s in ${prov.desc}" -> ty.toLoc
                  :: fieldNames.map(tp => msg"Declared at" -> tp.toLoc)
              )(raise, prov)
            case _ =>
          }
          RecordType.mk(fs.map { nt =>
            if (nt._1.name.head.isUpper)
              err(
                msg"Field identifiers must start with a small letter",
                nt._1.toLoc
              )(raise, prov)
            nt._1 -> rec(nt._2)
          })(prov)
        case Function(lhs, rhs) =>
          FunctionType(rec(lhs), rec(rhs))(tp(ty.toLoc, "function type"))
        case WithExtension(b, r) =>
          WithType(
            rec(b),
            RecordType(r.fields.mapValues(rec))(tp(r.toLoc, "extension record"))
          )(tp(ty.toLoc, "extension type"))
        case Literal(lit) =>
          ClassTag(lit, Set.single(lit.baseClass))(tp(ty.toLoc, "literal type"))
        case TypeName(name) =>
          val tyLoc = ty.toLoc
            ...
            typeNamed(tyLoc, name) match {
              ...
              case L(e) =>
                if (name.isEmpty || !name.head.isLower) e()
                  ...
                  typeNamed(tyLoc, name.capitalize) match {
                    case R(td) =>
                      td.kind match {
                        case Cls =>
                          clsNameToNomTag(td)(tp(tyLoc, "class tag"), ctx)
                        case Trt =>
                          trtNameToNomTag(td)(tp(tyLoc, "trait tag"), ctx)
                        case Als =>
                          err(
                            msg"Type alias $name cannot be used as a type tag",
                            tyLoc
                          )(raise, tpr)
                      }
                    case L(_) => e()
        ...
        case tv: TypeVar =>
          ...
          recVars.getOrElse(
            ...
            localVars
              .getOrElseUpdate(
                tv,
                new TypeVariable(ctx.lvl, Nil, Nil, tv.identifier.toOption)(
                  noProv
                )
              )
        ...
        case AppliedType(base, targs) =>
          ...
          typeNamed(ty.toLoc, base.name) match {
            case R(td) =>
              ...
              val realTargs =
                if (targs.size === tpnum) targs.map(rec)
                else {
                  err(
                    msg"Wrong number of type arguments – expected ${tpnum.toString}, found ${targs.size.toString}",
                    ty.toLoc
                  )(raise, prov)
                  (targs.iterator.map(rec) ++ Iterator.continually(
                    freshVar(noProv)
                  )).take(tpnum).toList
                }
        ...
        case Recursive(uv, body) =>
          val tv =
            freshVar(tp(ty.toLoc, "local type binding"), uv.identifier.toOption)
          val bod = rec(body)(ctx, recVars + (uv -> tv))
          tv.upperBounds ::= bod
          tv.lowerBounds ::= bod
          tv
        case Rem(base, fs) =>
          Without(rec(base), fs.toSet)(
            tp(ty.toLoc, "function type")
          ) // TODO use ty's prov
      }
    rec(ty)(ctx, Map.empty)
  }

  def typePattern(pat: Term)(implicit
      ctx: Ctx,
      raise: Raise,
      vars: Map[Str, SimpleType] = Map.empty
  ): SimpleType =
  ...
  def typeStatement(s: DesugaredStatement, allowPure: Bool)(implicit
      ctx: Ctx,
      raise: Raise
  ): PolymorphicType \/ Ls[Binding] = s match {
      ...
      if (!allowPure) {
        if (t.isInstanceOf[Var] || t.isInstanceOf[Lit])
          ...
          constrain(
            ...
            raise = err =>
              raise(
                Warning( // Demote constraint errors from this to warnings
    ...
    case _ =>
      err(msg"Illegal position for this ${s.describe} statement.", s.toLoc)(
        raise,
        noProv
      )
      R(Nil)
  ...
  def typeLetRhs(isrec: Boolean, nme: Str, rhs: Term)(implicit
      ctx: Ctx,
      raise: Raise,
      vars: Map[Str, SimpleType] = Map.empty
  ): PolymorphicType = {
    val res = if (isrec) {
      ...
      constrain(ty, e_ty)(
        raise,
        TypeProvenance(rhs.toLoc, "binding of " + rhs.describe),
        ctx
      )
      e_ty
  ...
  object ValidVar {
    ...
    def unapply(v: Var)(implicit raise: Raise): S[Str] = S {
      if (reservedNames(v.name))
        err(
          s"Illegal use of ${if (v.name.head.isLetter) "keyword" else "operator"}: " + v.name,
          v.toLoc
        )(raise, ttp(v))
      v.name
    }
  ...
  object ValidPatVar {
    ...
    def unapply(v: Var)(implicit ctx: Ctx, raise: Raise): Opt[Str] =
      if (ctx.inPattern && v.isPatVar) {
        ctx.parent
          .dlof(_.get(v.name))(N)
          .map(_.instantiate(0).unwrapProxies) |>? { case S(ClassTag(Var(v.name), _)) =>
          warn(
            msg"Variable name '${v.name}' already names a symbol in scope. " +
              s"If you want to refer to that symbol, you can use `scope.${v.name}`; " +
              s"if not, give your future readers a break and use another name :^)",
            v.toLoc
          )
        }
        ValidVar.unapply(v)
  ...
  def typeTerm(term: Term)(implicit
      ctx: Ctx,
      raise: Raise,
      vars: Map[Str, SimpleType] = Map.empty
  ): SimpleType =
    trace(s"$lvl. Typing ${if (ctx.inPattern) "pattern" else "term"} $term") {
      ...
      term match {
        case v @ Var(
              "_"
            ) => // TODO parse this differently... or handle consistently everywhere
        ...
        case Asc(trm, ty) =>
          ...
          if (ctx.inPattern)
            con(
              ...
              trm_ty,
              ty_ty
            ) // In patterns, we actually _unify_ the pattern and ascribed type
          else ty_ty
        case (v @ ValidPatVar(nme)) =>
          val prov =
            tp(if (verboseConstraintProvenanceHints) v.toLoc else N, "variable")
          ctx.env
            .get(nme)
            .map(
              _.instantiate
        ...
        case v @ ValidVar(name) =>
          val ty = ctx
            ...
            .getOrElse {
              ...
              (
                ctx.tyDefs.get(name),
                ctx.mthDecls.get(name).map(_ -- ctx.mthDefs(name).keys)
              ) match {
                case (S(td), S(abstMths)) if abstMths.nonEmpty =>
                  err(
                    (msg"Instantiation of an abstract type is forbidden"       -> term.toLoc)
                      :: (msg"Note that ${td.kind.str} ${td.nme} is abstract:" -> td.toLoc)
                      :: abstMths.map { case mn -> mthTy =>
                        msg"Hint: method $mn is abstract" -> mthTy.prov.loco
                      }.toList
                  )
        ...
        case Sel(obj, fieldName) =>
          ...
          def mthCall(obj: Term, fieldName: Var) =
            ctx.get(
              if (fieldName.name.contains('.')) fieldName.name
              else "." + fieldName.name
            ) match {
              case S(mth_ty) =>
                val o_ty = typeTerm(obj)
                ...
                con(
                  mth_ty.instantiate,
                  FunctionType(singleTup(o_ty), res)(prov),
                  res
                )
              case N =>
                if (fieldName.name.isCapitalized) {
                  if (ctx.methodBase.isDefinedAt(fieldName.name))
                    err(
                      msg"Implicit call to method ${fieldName.name} is forbidden because it is ambiguous."
                        -> term.toLoc :: msg"Unrelated methods named ${fieldName.name} are defined by:"
                        -> N :: ctx.tyDefs.valuesIterator
                          .filter(td =>
                            (td.mthDecls ++ td.mthDefs)
                              .exists(_.nme.name === fieldName.name)
                          )
                          .map { td =>
          ...
          obj match {
            case Var(name) if name.isCapitalized && ctx.tyDefs.isDefinedAt(name) =>
              ctx.get(s"$name.${fieldName.name}") match {
                ...
                case N =>
                  err(
                    msg"Class $name has no method ${fieldName.name}",
                    term.toLoc
                  )
        ...
        case Bind(l, r) =>
          val l_ty = typeTerm(l)
          val newCtx =
            ctx.nest // so the pattern's context don't merge with the outer context!
        ...
        case Test(l, r) =>
          ...
          val newCtx = ctx.nest
          val r_ty =
            typePattern(r)(newCtx, raise) // TODO make these bindings flow
        ...
        case CaseOf(s, cs) =>
          ...
          val req = tys.foldRight(BotType: SimpleType) { case ((a_ty, tv), req) =>
            a_ty & tv | req & a_ty.neg()
          }
          con(s_ty, req, cs_ty)
      }
    }(r => s"$lvl. : $r")

  def typeArms(scrutVar: Opt[Var], arms: CaseBranches)(implicit
      ctx: Ctx,
      raise: Raise,
      lvl: Int
    ...
    case Case(pat, bod, rest) =>
      val patTy = pat match {
        case lit: Lit =>
          ClassTag(lit, Set.single(lit.baseClass))(
            tp(pat.toLoc, "literal pattern")
          )
        case Var(nme) =>
          ...
          ctx.tyDefs.get(nme) match {
            ...
            case Some(td) =>
              td.kind match {
                case Als =>
                  err(msg"can only match on classes and traits", pat.toLoc)(
                    raise,
                    tpr
                  )
                case Cls =>
  ...
  def typeTerms(
      term: Ls[Statement],
      rcd: Bool,
      fields: List[Opt[Var] -> SimpleType]
    ...
    term match {
        ...
        val ty = {
          trm match {
            case Bra(false, t) if ctx.inPattern => // we use syntax `(x: (p))` to type `p` as a pattern and not a type...
              typePattern(t)
            case _ =>
              ctx.copy(inPattern = ctx.inPattern && no.isEmpty) |> { implicit ctx => // TODO change this?
      ...
      case (trm: Term) :: Nil =>
        if (fields.nonEmpty)
          warn(
            "Previous field definitions are discarded by this returned expression.",
            trm.toLoc
          )
        typeTerm(trm)
      // case (trm: Term) :: Nil =>
      ...
      case s :: sts =>
        ...
        diags.foreach(raise)
        val newBindings =
          desug.flatMap(typeStatement(_, allowPure = false).toOption)
  ...
  def expandType(
    ...
    def go(st: SimpleType, polarity: Boolean)(implicit
        inProcess: Set[SimpleType -> Bool]
    ): Type = {
      val st_pol = st -> polarity
      if (inProcess(st_pol))
        recursive.getOrElseUpdate(
          st_pol,
          freshVar(
            st.prov,
            st |>?? { case tv: TypeVariable =>
              tv.nameHint
            }
          )(0).asTypeVar
        )
      else
        (inProcess + st_pol).pipe { implicit inProcess =>
          st match {
            ...
            case tv: TypeVariable =>
              ...
              if (inProcess(bound -> polarity))
                recursive.getOrElseUpdate(
                  bound -> polarity,
                  freshVar(st.prov, tv.nameHint)(0).asTypeVar
                )
              else {
                ...
                recursive.get(st_pol) match {
                  case Some(variable) =>
                    Recursive(
                      ...
                      boundTypes
                        .reduceOption(mrg)
                        .getOrElse(if (polarity) Bot else Top)
                    )
            ...
            case _ =>
              val res = st match {
                case FunctionType(l, r) =>
                  Function(go(l, !polarity), go(r, polarity))
                case ComposedType(true, l, r) =>
                  Union(go(l, polarity), go(r, polarity))
                case ComposedType(false, l, r) =>
                  Inter(go(l, polarity), go(r, polarity))
                case RecordType(fs) =>
                  Record(fs.map(nt => nt._1 -> go(nt._2, polarity)))
                case TupleType(fs) =>
                  Tuple(fs.map(nt => nt._1 -> go(nt._2, polarity)))
                ...
                case TypeBounds(lb, ub) =>
                  if (polarity) go(ub, true) else go(lb, false)
                case Without(base, names) =>
                  Rem(go(base, polarity), names.toList)
