...
abstract class TyperDatatypes extends TyperHelpers { Typer: Typer =>
  ...
  class TupleSetConstraints(var constraints: Ls[Ls[ST]], var tvs: Ls[(Bool, ST)])(val prov: TypeProvenance) {
    def updateImpl(index: Int, bound: ST)(implicit raise: Raise, ctx: Ctx) : Unit = {
      val (cs0, u0) = constraints.flatMap { c =>
        TupleSetConstraints.lcg(tvs(index)._1, bound, c(index))(prov, ctx).map(u => (c, u))
      }.unzip
      val u = u0.map {
        case (x,y) => (x, y.groupBy(_._1).view.mapValues { l =>
          val p = l.head._1._1
          List(l.map(_._2).reduce[ST]((x,y) => ComposedType(!p,x,y)(noProv)))
        }.toMap)
      }
      constraints = cs0
      if (!u0.isEmpty) {
  ...
  object TupleSetConstraints {
    ...
    def lcg(pol: Bool, first: ST, rest: ST)
      (implicit prov: TypeProvenance, ctx: Ctx)
        : Opt[(Ls[(Bool, ST)], Ls[(Bool, ST) -> ST])] = (first.unwrapProxies, rest.unwrapProxies) match {
      case (a, ComposedType(p,l,r)) if p =/= pol =>
        for {
          (ll,lm) <- lcg(pol,a,l)
          (rl,rm) <- lcg(pol,a,r)
        } yield {
          (ll ++ rl,((pol, first) -> rest)::lm ++ rm)
        }
      case (a: TV, b: TV) if a.compare(b) === 0 => S(Nil, Nil)
      case (a: TV, b) => S(List((pol, a)), List((pol, a) -> b))
      case (a, b: TV) => S(List((pol, a)), List((pol, a) -> b))
      ...
      case (a: TupleType, b: TupleType) if a.fields.sizeCompare(b.fields) === 0 =>
        ...
        if (!fs.contains(N)) {
          val (l, m) = fs.flatten.reduce[(Ls[(Bool, ST)], Ls[(Bool, ST) -> ST])] {
    ...
    def mk(ov: Overload, f: FT)(implicit raise: Raise, ctx: Ctx): Opt[TupleSetConstraints] = {
      val u = ov.alts.map(lcgFunction(false, f, _)(ov.prov, ctx)).flatten.map {
        case (x,y) => (x, y.groupBy(_._1).view.mapValues { l =>
          val p = l.head._1._1
          List(l.map(_._2).reduce[ST]((x,y) => ComposedType(!p,x,y)(noProv)))
        }.toMap)
