...
abstract class TyperDatatypes extends TyperHelpers { self: Typer =>
  ...
  case class TypeProvenance(loco: Opt[Loc], desc: Str) {
    ...
    def &(that: TypeProvenance): TypeProvenance =
      this // arbitrary; maybe should do better

    override def toString: Str = "‹" + loco.fold(desc)(desc + ":" + _) + "›"

  }

  /** A type that potentially contains universally quantified type variables, and which can be isntantiated to a given
    * level.
    */
  ...
  /** A type with universally quantified type variables (by convention, those variables of level greater than `level`
  ...
  case class PolymorphicType(level: Int, body: SimpleType) extends TypeScheme {
    ...
    def rigidify(implicit lvl: Int): SimpleType =
      freshenAbove(level, body, rigidify = true)
  ...
  case class FunctionType(lhs: SimpleType, rhs: SimpleType)(
      val prov: TypeProvenance
  ) extends MiscBaseType {
  ...
  case class RecordType(fields: List[(Var, SimpleType)])(
      val prov: TypeProvenance
  ) extends SimpleType {
    ...
    def toInter: SimpleType =
      fields
        .map(f => RecordType(f :: Nil)(prov))
        .foldLeft(TopType: SimpleType)(((l, r) => ComposedType(false, l, r)(noProv)))
    ...
    override def toString =
      s"{${fields.map(f => s"${f._1}: ${f._2}").mkString(", ")}}"
  ...
  object RecordType {
    ...
    def mk(fields: List[(Var, SimpleType)])(
        prov: TypeProvenance = noProv
    ): SimpleType =
      if (fields.isEmpty) ExtrType(false)(prov) else RecordType(fields)(prov)

  }

  case class TupleType(fields: List[Opt[Var] -> SimpleType])(
      val prov: TypeProvenance
  ) extends MiscBaseType {
    ...
    lazy val toRecord: RecordType =
      RecordType(
        fields.zipWithIndex.map { case ((_, t), i) =>
          (Var("_" + (i + 1)), t)
        } ::: // TODO dedup fields!
  ...
  case class ComposedType(pol: Bool, lhs: SimpleType, rhs: SimpleType)(
      val prov: TypeProvenance
  ) extends SimpleType {
  ...
  case class Without(base: SimpleType, names: Set[Var])(
      val prov: TypeProvenance
  ) extends MiscBaseType {
  ...
  /** A proxy type is a derived type form storing some additional information, but which can always be converted into an
  ...
  /** A proxy type, `S with {x: T; ...}` is equivalent to `S\x\... & {x: T; ...}`.
    */
  case class WithType(base: SimpleType, rcd: RecordType)(
      val prov: TypeProvenance
  ) extends ProxyType {
  ...
  case class TypeRef(defn: TypeDef, targs: Ls[SimpleType])(
    ...
    def expandWith(paramTags: Bool)(implicit raise: Raise): SimpleType = {
      val body_ty = typeType(defn.body)(
        ctx,
        raise,
        vars = defn.tparams.map(_.name).zip(targs).toMap
      )
      lazy val tparamTags =
        if (paramTags)
          RecordType.mk(
            defn.tparams
              .lazyZip(targs)
              .map((tp, tv) => tparamField(defn.nme, tp) -> FunctionType(tv, tv)(noProv))
              .toList
          )(noProv)
        else TopType
      defn.kind match {
        case Als => body_ty
        case Cls =>
          clsNameToNomTag(defn)(noProv /*TODO*/, ctx) & body_ty & tparamTags
        case Trt =>
          trtNameToNomTag(defn)(noProv /*TODO*/, ctx) & body_ty & tparamTags
      }
    }

    override def toString = {
      val displayName =
        if (primitiveTypes.contains(defn.nme.name)) defn.nme.name.capitalize
        else defn.nme.name
      if (targs.isEmpty) displayName
      else s"$displayName[${targs.mkString(",")}]"
    }
  ...
  case class ClassTag(id: SimpleTerm, parents: Set[Var])(
      val prov: TypeProvenance
  ) extends BaseType
      with ObjectTag {

    lazy val parentsST =
      parents.map(identity[SimpleTerm]) // TODO inefficient... improve
  ...
  /** `TypeBounds(lb, ub)` represents an unknown type between bounds `lb` and `ub`. The only way to give something such
    * a type is to make the type part of a def or method signature, as it will be replaced by a fresh bounded type
    * variable upon subsumption checking (cf rigidification).
    */
  case class TypeBounds(lb: SimpleType, ub: SimpleType)(
      val prov: TypeProvenance
  ) extends SimpleType {
  ...
  object TypeBounds {
    ...
    def mk(
      ...
      if ((lb.is(ub)) || lb === ub || lb <:< ub && ub <:< lb) lb
      else TypeBounds(lb, ub)(prov)

  }

  /** A type variable living at a certain polymorphism level `level`, with mutable bounds. Invariant: Types appearing in
    * the bounds never have a level higher than this variable's `level`.
    */
