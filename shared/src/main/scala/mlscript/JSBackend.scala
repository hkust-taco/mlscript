...
class JSBackend {
  ...
  def deconstructIfApplications(lhs: Term, rhs: Term): Opt[(Term, Term, Term)] =
    lhs match {
      case App(App(Var("if"), rhs2), rhs1) => Some((rhs2, rhs1, rhs))
      case _                               => None
    }
  ...
  def translateTerm(term: Term): Str = term match {
    ...
    case CaseOf(term, cases) =>
      "(() => {" + translateCaseBranch("x", cases).mkString(
        "\n"
      ) + s"})(${translateTerm(term)})"
    case IntLit(value) =>
