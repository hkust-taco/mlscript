...
class JSBackend(pgrm: Pgrm) {
  ...
  /**
   * This function returns a tuple. The first element is the prelude code, which
   * should be executed without reporting the results. The second element is the
   * list of executions. Each execution may be multiple lines.
   */
  def testCode(): (Ls[Str], Ls[Str -> Ls[Str]]) = {
    val (diags, (typeDefs, otherStmts)) = pgrm.desugared
    ...
    // This hash map counts how many times a name has been used.
    val resolveShadowName = new ShadowNameResolver
    val classDefs: Ls[JSStmt] = typeDefs
      .map { case TypeDef(kind, TypeName(name), typeParams, actualType, _, mthDefs) =>
        kind match {
          case Cls =>
            classNames += name
    ...
    val stmtGroups: Ls[Str -> Ls[JSStmt]] = otherStmts.map {
      case Def(isRecursive, originalName, L(body)) =>
        val translatedBody = translateTerm(body)(topLevelScope)
        // `declName` means the name used in the declaration.
        // We allow define functions with the same name as classes.
        ...
        // TODO: update here after fixing the re-declaration.
        // TODO: remove the use of `ShadowNameResolver` here.
        originalName -> (if (resolveShadowName(originalName) === originalName) {
                           // First time: `let <declName> = <expr>;`
                           JSLetDecl(declName, translatedBody) ::
                             JSExprStmt(JSIdent(declName)) :: Nil
                         } else {
                           // Not first time, just re-assign.
                           JSExprStmt(JSAssignExpr(JSIdent(declName), translatedBody)) :: Nil
                         })
      // Ignore type declarations.
      case Def(isRecursive, name, R(body)) => "" -> Nil
      // Just execute the expression.
      case term: Term =>
        "res" -> (JSExprStmt(translateTerm(term)(topLevelScope)) :: Nil)
    }
    val prelude = if (hasWithConstruct) {
      JSBackend.makeWithConstructDecl(withConstructFnName).toSourceCode.lines map { _.toString }
    } else { Nil }

    val prologue = prelude ::: (classDefs flatMap { _.toSourceCode.lines map { _.toString } })
    val executions = stmtGroups map { case name -> stmts =>
      name -> (SourceCode.concat(stmts map { _.toSourceCode }).lines map { _.toString })
    }
    (prologue, executions)
  }
}
