          ...
          trace(s"${lvl}. Typing parent spec $p") {
            ...
            info match {
              ...
              case rawMxn: TypedNuMxn =>
                ...
                val argMembs =  {                      
                  ...
                  val paramMems = mxn.params.lazyZip(parArgs).map {
                    case (nme -> p, _ -> Fld(_, _, _, a)) => // TODO check name, mut, spec
              ...
              case rawCls: TypedNuCls =>
                ...
                val paramMems = cls.params.lazyZip(parArgs).map { case (nme -> p, _ -> Fld(_, _, _, a)) => // TODO check name, mut, spec
                  implicit val genLambdas: GenLambdas = true
                  val a_ty = typeTerm(a)
                  p.lb.foreach(constrain(_, a_ty))
                  constrain(a_ty, p.ub)
      ...
      decl match {
        case td: NuTypeDef =>
          td.params.getOrElse(Tup(Nil)).fields.map {
            case (S(nme), Fld(mut, spec, _, value)) =>
