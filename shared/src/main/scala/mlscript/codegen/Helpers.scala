  ...
  def inspect(t: Terms): Str = t match {
    ...
    case Tup(fields) =>
      val entries = fields map {
        case (S(name), Fld(_, _, _, value)) => s"$name: ${inspect(value)}"
        case (N, Fld(_, _, _, value))       => s"_: ${inspect(value)}"
      }
      s"Tup(${entries mkString ", "})"
    case Rcd(fields) =>
      val entries = fields.iterator
        .map { case k -> Fld(_, _, _, v) => s"${inspect(k)} = ${inspect(v)}" }
        .mkString(", ")
      s"Rcd($entries)"
    ...
    case Splc(fs)       => 
      val elems = fs.map{case L(l) => s"...${inspect(l)}" case R(Fld(_, _, _, r)) => inspect(r)}.mkString(", ")
      s"Splc($elems)"
