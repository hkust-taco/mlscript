...
class ToTsTypegenContext(
  ...
  /**
    * Polarity of a type is flipped if it's at function input.
    * No change if it's None.
    *
    * @param newPolarity
    */
    ...
    this match {
      ...
      case Neg(base) =>
        ...
        if (ctx.polarity =/= Some(false)) {
          SourceCode("ERROR: cannot convert negative type in non-negative polarity position")
        }
      ...
      case Bounds(lb, ub) =>
        ctx.polarity match {
          ...
          case Some(true) => ub.toTsType
          // negative polarity takes lower bound
          case Some(false) => lb.toTsType
          // TODO: Yet to handle invariant types
          case None => SourceCode("TODO") ++ lb.toTsType ++ ub.toTsType
        }
      case WithExtension(base, rcd) =>
        Inter(Rem(base, rcd.fields.map(tup => tup._1)), rcd).toTsType
