...
final case class TypingUnit(entities: Ls[Statement], depList: Ls[Import] = Nil) extends TypingUnitImpl
...
final case class NuFunDef(
  ...
  tparams: Ls[TypeName],
  rhs: Term \/ Type,
)(val declareLoc: Opt[Loc], val exportLoc: Opt[Loc], val signature: Opt[NuFunDef], val outer: Opt[Outer]) extends NuDecl with DesugaredStatement {
  val body: Located = rhs.fold(identity, identity)
